...
if (!isset($newData['test_no']) || !isset($newData['performer_name']) || !isset($newData['section']) || !isset($newData['test_type'])) {
...
$record = [
    ...
    'test_type' => $newData['test_type'],
    'timestamp' => date('Y-m-d H:i:s'), // Current date and time: 2025-09-16 16:04 PKT
];

// Add test-specific fields based on test type
switch ($newData['test_type']) {
    ...
    default:
        echo json_encode(["status" => "error", "message" => "Unknown test type"]);
        exit;
}
